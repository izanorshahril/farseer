...
Six tests in `farseer-api` are `#[ignore]`d: each spawns a real headless `claude` process, and `AGENTS.md`'s own "a one-word prompt cost $0.32 loading plugins" finding means six of those in one run is real minutes and real cost, not a hang.
`cargo test --workspace` skips them; run `cargo test -p farseer-api --lib -- --ignored` to include them.
