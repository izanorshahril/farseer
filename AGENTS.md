...
`farseer-runner` resolves `claude`, builds its argv, spawns and reaps it under a Job Object, drains its stream-json output through the mapping, and creates and tears down worktrees per `04` (reap first, delete with backoff, no rename-quarantine - `04` disproved it). `farseer-manager` calls all of that for one `WorkerContract`, appending its events, finalizing its run row, and exposing a `LivenessHandle` and `CancelToken` for `18`/`05`'s watchdog and cancellation - the watchdog itself never calls `cancel`, per `05`'s no-auto-kill rule, and `cancel` does not yet produce `05`'s `Cancelled` outcome (the row reads `failed`). `farseer-api` calls it: `POST /v1/cells/{id}/instruct` creates a worktree or plain directory per the cell's `workspace_strategy`, runs the cell's manager against a goal (never a roster worker - only `claude-code` is wired, and only the manager is guaranteed to name it), and tears the workspace down after; `POST /v1/runs/{id}/cancel` ends a run early. A `Worktree` cell's runs are worktrees of wherever `farseer serve` was started - `13` keeps no git flag on `CellDefinition`, so there is nowhere else for that path to come from yet. There is still no manager loop deciding delegation and no other manager verb.
