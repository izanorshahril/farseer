...
`farseer-runner` resolves `claude`, builds its argv, spawns and reaps it under a Job Object, and drains its stream-json output through the mapping. `farseer-manager` calls all of that for one `WorkerContract`, appending its events, finalizing its run row, and exposing a `LivenessHandle` and `CancelToken` for `18`/`05`'s watchdog and cancellation - the watchdog itself never calls `cancel`, per `05`'s no-auto-kill rule, and `cancel` does not yet produce `05`'s `Cancelled` outcome (the row reads `failed`). `farseer-api` now calls it: `POST /v1/cells/{id}/instruct` runs a cell's manager against a goal (never a roster worker - only `claude-code` is wired, and only the manager is guaranteed to name it) and `POST /v1/runs/{id}/cancel` ends it. There is still no manager loop deciding delegation, no other manager verb, and no real workspace isolation - every run gets a fresh plain directory, not a worktree.
